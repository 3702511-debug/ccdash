...
6. **Открывает Системные настройки → Конфиденциальность и безопасность → Полный доступ к диску** и ждёт, пока пользователь добавит туда `bun` (см. ниже почему это критично)

### Б1.5. Full Disk Access для bun (обязательно!)

Когда setup-local.ts откроет панель FDA, пользователь должен:

1. Нажать `+` под списком
2. В Finder-диалоге: `Shift+Cmd+G` → ввести путь к bun (его покажет setup-скрипт, обычно `/opt/homebrew/bin/bun`)
3. Выбрать файл `bun` → «Открыть»
4. Включить слайдер напротив `bun`
5. Вернуться в терминал, нажать Enter, чтобы setup продолжил

**Без этого** launchd-запущенный bun не имеет прав на чтение `~/.claude/projects/` → дашборд видит 0 сессий, хотя сессии Claude Code реально работают в Terminal. Поэтому шаг обязательный.

Скрипт после Enter сам сделает `launchctl kickstart` чтобы новый bun-процесс подхватил TCC-права.
...
- **Не редактируй файлы внутри git-клона** (`server.ts`, `setup-local.ts`, `RUNBOOK.md`, `RELEASE.json`, `CLAUDE.md` и пр.). Этот код централизованно поддерживается автором — любые правки потеряются при следующем обновлении (setup-local.ts перепишет runtime), либо сломают `git pull --ff-only` (диверженция). Если пользователь просит изменить поведение дашборда — направь его к автору репо. Подробное правило для AI-агента — в `CLAUDE.md` в корне репо.
- **Можно править** (это вне репо): `~/.cc-dashboard/auth.json`, `vapid.json`, `auto-update.flag`, `tunnel-watchdog.sh`, plist'ы LaunchAgent'ов.
...
## Safe rm (защита от rm -rf)

С v1.0.34 `setup-local.ts` добавляет в `~/.bash_profile` и `~/.zshrc` обёртку `rm`, которая отправляет файлы в Корзину Finder через `/usr/bin/trash` вместо безвозвратного `unlink`. Это покрывает класс аварий типа `rm -rf ~/.claude` — файлы можно восстановить из Корзины (Finder → Корзина → правый клик → «Вернуть»).

Срабатывает **только в интерактивных шеллах** (новые Terminal-табы или после `source ~/.bash_profile` / `source ~/.zshrc`). Скрипты, которые используют `rm` напрямую, не затронуты.

**Bypass для реального unlink (если нужно навсегда):**
- `\rm файл` — escape алиаса
- `/bin/rm файл` — прямой путь к системному rm
- `command rm файл` — обход функции

**Отключить полностью:** найди блок `# === Safe rm (cc-dashboard) ===` в `~/.bash_profile` и `~/.zshrc`, удали его (до конца файла или до следующей секции). При следующем `setup-local.ts` (через апдейт дашборда) блок установится заново — если хочешь убрать насовсем, удали и добавь после блока в RC-файле любой свой комментарий с тем же маркером, чтобы скрипт думал что уже установлено.

**Ограничения:**
- `/usr/bin/trash` доступен в macOS 14 (Sonoma) и новее. На старых версиях safe-rm молча пропускается.
- Файлы из Корзины автоматически удаляются через 30 дней (если включена авточистка в настройках Finder).
